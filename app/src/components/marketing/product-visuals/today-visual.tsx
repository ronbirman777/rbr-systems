 ...
 * rebuilt to match the supplied product reference screenshot directly (not
 * the real TodayScreen component - see the module doc comment history in
 * this file's git log for why that couldn't carry the premium treatment
 * without editing product code). Photo header + greeting, an intention
 * card, and one "Happening Now" card in Deep Forest - the ONE deliberate
 * dark surface, used as a spotlight for the live/current item, exactly as
 * the reference does it. Everything else stays light. Shows fewer, larger
 * elements rather than compressing many rows, per the marketing-scale rule.
 */
const RADIUS_CLASS = {
...
export function TodayVisual({
  ...
  return (
    <div className="w-full h-full bg-idw-parchment flex flex-col overflow-y-auto no-scrollbar">
      ...
      <div className="p-3.5 flex flex-col gap-3">
        <div className={`${r.card} bg-white/80 px-4 py-3.5 shadow-sm`}>
        ...
        {upNextLabel && (
          <div className="font-ui text-[9px] font-semibold uppercase tracking-[0.14em] text-idw-forest/35">
            Up Next · {upNextLabel}
          </div>
        )}
      </div>
