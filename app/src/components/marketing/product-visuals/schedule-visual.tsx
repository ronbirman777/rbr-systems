 ...
 * matching the supplied product reference: a six-day selector with a small
 * dot marking the current day, a vertical timeline (dot + connecting line),
 * the current session spotlighted in Deep Forest exactly like Today's
 * "Happening Now" card, and the persistent bottom navigation. Fewer,
 * larger session cards rather than many compressed rows.
 */
export type ScheduleVisualDay = { label: string; day: string; selected?: boolean; isToday?: boolean };
...
export function ScheduleVisual({
  ...
  return (
    <div className="w-full h-full bg-idw-parchment flex flex-col">
        ...
        <div className="flex gap-1.5">
          {days.map((d) => (
            ...
            >
              ...
              <div className="font-ui text-[12px] font-medium flex items-center justify-center gap-1">
                {d.day}
                {d.isToday && (
                  <span
                    className={`w-1 h-1 rounded-full ${d.selected ? "bg-idw-parchment" : "bg-idw-clay"}`}
                    aria-hidden="true"
                  />
                )}
              </div>
      ...
      {showNav && <GuestNav active="Schedule" />}
    </div>
  );
}
